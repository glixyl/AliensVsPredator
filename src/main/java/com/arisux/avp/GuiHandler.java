...
public class GuiHandler implements IGuiHandler, IInitializable
{
	...
	{
		if (ID == AliensVsPredator.INSTANCE.properties.GUI_ASSEMBLER)
			return ((TileEntityAssembler) world.getTileEntity(x, y, z)).getNewContainer(player);

		if (ID == AliensVsPredator.INSTANCE.properties.GUI_TURRET)
			return ((TileEntityTurret) world.getTileEntity(x, y, z)).getNewContainer(player);

		if (ID == AliensVsPredator.INSTANCE.properties.GUI_WRISTBRACER && player != null && player.getCurrentEquippedItem() != null)
		{
	...
	public Object getClientGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z)
	{
		...
		if (ID == AliensVsPredator.INSTANCE.properties.GUI_WRISTBRACER)
		{
			Item item = player.getCurrentEquippedItem().getItem();
